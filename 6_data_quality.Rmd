...
myfunction <- function(myfile){
  ...
  dt <- dt %>% 
    mutate(
      ...
      CV = map_dbl(data, function(t){
        t$group[!grepl("QC|solv", t$group)] <- "study"
        a <- t %>%
          filter(group %in% c("study", "QC")) %>%
          group_by(group) %>%
          summarise(CV = CV(val_imp))
        out <- a$CV[1] < a$CV[2]
        return(out)
      })) %>%
...
for(i in c("t", "m")){
  print(i)
  print(table(dt[,paste(i, "CV", sep = "_")]))
...
quantile(c(dt$t_linearity, dt$m_linearity), na.rm = TRUE)

dt$t_CV <-  as.numeric(sprintf("%.1f", as.numeric(dt$t_CV)))
dt$t_linearity <-  as.numeric(sprintf("%.3f", as.numeric(dt$t_linearity)))
dt$t_ES <-  as.numeric(sprintf("%.1f", as.numeric(dt$t_ES)))
dt$m_CV <-  as.numeric(sprintf("%.1f", as.numeric(dt$m_CV)))
...
DT::datatable(
  dt[,c("cmp", "name", "t_linearity", "t_ES", "m_linearity", "m_ES")], 
