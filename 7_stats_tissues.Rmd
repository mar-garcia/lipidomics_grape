...
dt <- dt %>%
  mutate(CV = map_dbl(data_pqn, function(t){
    t %>% 
...
data <- dt %>%
  ...
  select("cmp", "sample", "val_pqn_imp") %>%
  pivot_wider(names_from = "cmp", values_from = "val_pqn_imp") %>%
  column_to_rownames("sample")
data <- log10(data)
...
pm <- pheatmap::pheatmap(
  data, cutree_rows = 3, cutree_cols = 3,
  annotation_col = dt_cmp, annotation_colors = mycols)

dtx <- cbind(pm$tree_col$labels, cutree(pm$tree_col, k = 3))
