...
cp <- chromPeaks(xchr)
if(s == "tissues"){
  cl <- gsub(".*_", "", gsub("_rep.*", "", basename(fileNames(ydata))))
} else if(s == "maturation"){
  cl <- gsub(
    "MIX.*", "", gsub(".*QC.*", "QC", gsub(
      ".*grape_", "", gsub("_Rep.*", "", basename(fileNames(ydata))))))
}
pdp <- PeakDensityParam(sampleGroups = cl,
                        minFraction = 1,
                        binSize = 0.02,
                        bw = 3)
ydatax <- groupChromPeaks(xchr, param = pdp)
ydatax <- adjustRtime(ydatax, param = pgp)
ydatax <- groupChromPeaks(ydatax, param = pdp)
yft <- as.data.frame(featureDefinitions(ydatax))
...
for(i in seq(nrow(ft))){
  ...
  if(s == "tissues" & p == "NEG"){
    # do not consider tail of 
    ### "PA_16:0_18:2":
    if(length(unlist(matchWithPpm(671.4657, ft$mzmed[i], ppm = 10))) == 1){
      if(ft$rtmed[i]/60 > 16.47 & ft$rtmed[i]/60 < 16.70){
        idx_ms2 <- NULL
      }}
    ### PA(18:0/18:2): 
    if(length(unlist(matchWithPpm(699.497, ft$mzmed[i], ppm = 10))) == 1){
      if(ft$rtmed[i]/60 > 16.85 & ft$rtmed[i]/60 < 17.06){
        idx_ms2 <- NULL
      }}
    
    ### 13C PA(18:0/18:2): 
    if(length(unlist(matchWithPpm(699.497 + 1.003355, 
                                  ft$mzmed[i], ppm = 10))) == 1){
      if(ft$rtmed[i]/60 > 16.85 & ft$rtmed[i]/60 < 17.06){
        idx_ms2 <- NULL
      }}
    ### PA(18:2/18:2)
    if(length(unlist(matchWithPpm(695.4657, ft$mzmed[i], ppm = 10))) == 1){
      if(ft$rtmed[i]/60 > 16.03 & ft$rtmed[i]/60 < 16.22){
        idx_ms2 <- NULL
      }}
    ...
    ### "PA_16:0_18:2":
    if(length(unlist(matchWithPpm(671.4657, ft$mzmed[i], ppm = 10))) == 1){
      if(ft$rtmed[i]/60 > 16.40 & ft$rtmed[i]/60 < 16.65){
        idx_ms2 <- NULL
      }}
  } else if(s == "tissues" & p == "POS"){
    # Isomers
    ### DAG_16:1_18:2
    if(length(unlist(matchWithPpm(608.5249, ft$mzmed[i], ppm = 10))) == 1){
      if(length(unlist(matchWithPpm(608.5249, ft$mzmed, ppm = 10))) == 2 &
         length(unlist(matchWithPpm(608.5249, yft$mzmed, ppm = 10))) == 2){
        rt1 <- min(yft[unlist(matchWithPpm(608.5249, yft$mzmed, 
                                           ppm = 10)),"rtmed"])
        rt2 <- max(yft[unlist(matchWithPpm(608.5249, yft$mzmed, 
                                           ppm = 10)),"rtmed"])
        rt <- data.frame(cbind(
          rt1 = abs(rt1 - rtime(ms2[idx_ms2])),
          rt2 = abs(rt2 - rtime(ms2[idx_ms2]))
        ))
        if(which.min(ft$rtmed[unlist(
          matchWithPpm(608.5249, ft$mzmed, ppm = 10))]) == 1){
          idx_ms2 <- idx_ms2[
            apply(rt, 1, which.min) == which(
              unlist(matchWithPpm(608.5249, ft$mzmed, ppm = 10)) == i)]
        } else {
          idx_ms2 <- idx_ms2[
            apply(rt, 1, which.min) != which(
              unlist(matchWithPpm(608.5249, ft$mzmed, ppm = 10)) == i)]
    ...
    ### DAG_18:1_18:1 / DAG_18:0_18:2 - (2)13C[M+NH4]+
    if(length(unlist(matchWithPpm(638.5718 + 1.003355*2, ft$mzmed[i], 
                                  ppm = 10))) == 1){
      if(length(unlist(matchWithPpm(638.5718 + 1.003355*2, ft$mzmed, 
                                    ppm = 10))) == 2 #&
         #length(unlist(matchWithPpm(638.5718 + 1.003355*2, yft$mzmed, ppm = 10))) == 2
      ){
        #rt1 <- min(yft[unlist(matchWithPpm(638.5718 + 1.003355*2, yft$mzmed, ppm = 10)),"rtmed"])
        #rt2 <- max(yft[unlist(matchWithPpm(638.5718 + 1.003355*2, yft$mzmed, ppm = 10)),"rtmed"])
        rt1 <- min(ft$rtmed[
          unlist(matchWithPpm(638.5718 + 1.003355*2, ft$mzmed, ppm = 10))])
        rt2 <- max(ft$rtmed[
          unlist(matchWithPpm(638.5718 + 1.003355*2, ft$mzmed, ppm = 10))])
        ...
        if(which.min(ft$rtmed[unlist(
          matchWithPpm(638.5718 + 1.003355*2, ft$mzmed, ppm = 10))]) == 1){
          idx_ms2 <- idx_ms2[
            apply(rt, 1, which.min) == which(
              unlist(matchWithPpm(638.5718 + 1.003355*2, ft$mzmed, 
                                  ppm = 10)) == i)]
        } else {
          idx_ms2 <- idx_ms2[
            apply(rt, 1, which.min) != which(
    ...
    ### Unk-026 13C[M+NH4]+
    if(length(unlist(matchWithPpm(896.7452 + 1.007276 + 17.02654 + 1.003355, 
                                  ft$mzmed[i], ppm = 10))) == 1){
      if(length(unlist(matchWithPpm(
        896.7452 + 1.007276 + 17.02654 + 1.003355, ft$mzmed, 
        ppm = 10))) > 1 &
        length(unlist(matchWithPpm(896.7452 + 1.007276 + 17.02654 + 1.003355,
                                   yft$mzmed, ppm = 10))) > 1){
        
        rt <- yft[unlist(matchWithPpm(
          896.7452 + 1.007276 + 17.02654 + 1.003355, yft$mzmed, 
          ppm = 10)),"rtmed"]
        rt1 <-rt[order(rt)][1]
        rt2 <-rt[order(rt)][2]
        rt3 <-rt[order(rt)][3]
        ...
        rt <- data.frame(cbind(
          rt1 = abs(rt1 - rtime(ms2[idx_ms2])),
          rt2 = abs(rt2 - rtime(ms2[idx_ms2])),
          rt3 = abs(rt3 - rtime(ms2[idx_ms2])),
          rt4 = abs(rt4 - rtime(ms2[idx_ms2]))
        ))
        
        idx_ms2 <- idx_ms2[
          apply(rt, 1, which.min) ==
            order(ft[unlist(matchWithPpm(
              896.7452 + 1.007276 + 17.02654 + 1.003355, ft$mzmed, 
              ppm = 10)),"rtmed"])[ft$rtmed[i] == ft[unlist(matchWithPpm(
                896.7452 + 1.007276 + 17.02654 + 1.003355, ft$mzmed, 
                ppm = 10)),"rtmed"]]]
      }
    }      
    # do not consider tail of 
    ### DAG_18:1_18:2:
    if(length(unlist(matchWithPpm(636.5562, ft$mzmed[i], ppm = 10))) == 1){
      if(ft$rtmed[i]/60 > 18.21 & ft$rtmed[i]/60 < 18.33){
        idx_ms2 <- NULL
      }}      
  }
  ...
  if(length(idx_ms2) > 0){
    for(j in seq(length(idx_ms2))){
      ...
      if((length(idx_cp) == 1 & cpx["sample"] == idy)){
        if((rtime(ms2)[idx_ms2][j] > rtmin) & 
           (rtime(ms2)[idx_ms2][j] < rtmax)){
...
adds <- data.frame(rbind(
  ...
  c("[M-H-CO2]-",       -1.007276 - 43.98982,  1, "NEG"), 
...
if(any(duplicated(tmp2))){
  ...
  for(i in dpl){
    ...
    if(nrow(i.tmp) > 1){
      if(length(unique(i.tmp$adduct)) == 1){
...
if(nrow(ms2_excl) > 0){
  ...
  if(s == "tissues" & p == "NEG"){
    excl <- c(
      ...
      "PA_18:2_20:0 [M-H]-", "PA_18:2_22:0 [M-H]-", "PA_18:2_22:0 _II [M-H]-", 
      "mPA_16:0_18:1 [M-H]-", "mPA_16:0_18:2 [M-H]-", 
      "mPA_18:0_18:1 [M-H]-", "mPA_18:0_18:2 [M-H]-", "mPA_18:1_18:2 [M-H]-",
      "mPA_18:2_18:2 [M-H]-", "mPA_18:2_18:3 [M-H]-",
      "dmPA_16:0_18:1 [M-H]-", "dmPA_16:0_18:2 [M-H]-", "dmPA_18:2_18:2 [M-H]-", 
      "PC_15:0_18:1d7 [M-H+HCOOH]-",
      "PE_15:0_18:1d7 [M-H]-", "PE_16:0_18:1 [M-H]-", "PE_16:0_18:3 [M-H]-", 
      "PE_18:1_18:2 [M-H]-", "PE_18:2_18:2 [M-H]-", "PE_18:2_18:3 [M-H]-",
      "PE_18:2_20:0 [M-H]-", "PE_18:2_22:0 [M-H]-", "PE_18:2_23:0 [M-H]-",
      "PG_15:0_18:1d7 [M-H]-", "PG_16:0_16:0 [M-H]-", "PG_16:0_18:1 [M-H]-", 
      "PG_16:0_18:2 [M-H]-",
      "PI_18:0_18:2 [M-H]-",
      "PS_18:2_20:0 [M-H]-",
      "CER_15:0_18:1d7 [M-H+HCOOH]-", 
      "CER_hydroxy_41:0 [M-H+HCOOH]-","CER_hydroxy_41:1 [M-H+HCOOH]-",
      "CER_dihydroxy_41:1 [M-H]-", "CER_dihydroxy_16:0_26:1 [M-H]-", 
      ...
      "CER_hex_dihydroxy_18:1_25:0 [M-H]-", "CER_hex_dihydroxy_18:1_26:0 [M-H]-", 
      "MGDG_18:1_18:2 [M-H+HCOOH]-", "MGDG_18:2_18:2 [M-H+HCOOH]-",
      "Malic acid [M-H]-",
      "Unk-001 _I [M-H]-", "Unk-004 [M-H]-", "Unk-014 [M-H]-", 
      "Unk-016 [M-H+HCOOH]-", "Unk-019 [M-H+HCOOH]-", "Unk-020 [M-H+HCOOH]-",
      "Unk-021 [M-H]-", "Unk-022 [M-H]-", "Unk-023 [M-H]-",
      "Unk-075 _14:2_15:3 [M-H]-", "Unk-122 _NL325 [M-H]-", 
      "Unk-134 _NL325 [M-H]-", "Unk-135 _NL325 [M-H]-",
      "Unk-173 [M-H+HCOOH]-", "Unk-175 _18:2 [M-H]-", "Unk-182 [M-H+HCOOH]-",
      "Unk-187 [M-H]-", "Unk-191 _16:0_18:1 [M-H]-","Unk-198 [M-H+HCOOH]-", 
      "Unk-202 [M-H]-", "Unk-213 _18:2 [M-H]-", "Unk-223 [M-H]-",
      "Unk-224 [M-H]-", "Unk-247 [M-H]-")
  } else if(s == "maturation" & p == "NEG"){
    excl <- c(
      excl, 
      "PA_16:0_18:1 [M-H]-", 
      "PA_18:0_18:2 [M-H]-", "PA_18:0_18:3 [M-H]-", "PA_18:1_18:2 [M-H]-",
      "PA_18:3_18:3 [M-H]-",
      "mPA_16:0_18:1 [M-H]-", "mPA_16:0_18:2 [M-H]-", "mPA_16:0_18:3 [M-H]-",
      "mPA_18:0_18:2 [M-H]-", "mPA_18:1_18:2 [M-H]-", "mPA_18:2_18:2 [M-H]-", 
      "mPA_18:2_18:3 [M-H]-",
      "LPC_18:1d7 [M-H+HCOOH]-", 
      "PC_17:0_18:2 [M-H+HCOOH]-", "PC_18:2_20:0 [M-H+HCOOH]-",
      "LPE_18:1d7 [M-H]-",
      "PE_15:0_18:1d7 [M-H]-", "PE_16:0_18:3 [M-H]-",  
      "PE_18:0_18:3 [M-H]-", "PE_18:1_18:2 [M-H]-", "PE_18:2_18:2 [M-H]-",
      "PE_18:2_18:3 [M-H]-", "PE_18:3_18:3 [M-H]-",
      "PE_18:2_20:0 [M-H]-", "PE_18:2_22:0 [M-H]-","PE_18:2_23:0 [M-H]-",
      "PG_15:0_18:1d7 [M-H]-", "PG_16:0_16:0 [M-H]-", "PG_16:0_18:1 [M-H]-",
      "PG_16:0_18:2 [M-H]-",
      "PI_16:0_18:1 [M-H]-",
      "CER_15:0_18:1d7 [M-H+HCOOH]-", 
      "CER_hex_hydroxy_34:2 [M-H]-",
      "CER_hex_dihydroxy_18:1_22:0 [M-H]-", "CER_hex_dihydroxy_18:1_24:0 [M-H]-",
      "CER_hex_dihydroxy_18:1_26:0 [M-H]-",
      "Unk-004 [M-H]-", "Unk-20 [M-H+HCOOH]-", 
      "Unk-134 _NL325 [M-H]-", "Unk-229 _I [M-H]-", "Unk-231 [M-H]-", 
      "Unk-232 _18:2 [M-H]-", "Unk-271 [M-H]-")
  } else if(s == "tissues" & p == "POS"){
    excl <- c(
      excl, 
      "LPC_16:0 [M+H]+",
      "PA_18:3_18:3 [M+NH4]+",
      "PE_15:0_18:1d7 [M+H]+", "PE_18:0_18:2 [M+H]+", "PE_18:1_18:2 [M+H]+", 
      "PE_18:2_18:2 [M+H]+",
      "PE_18:2_20:0 [M+H]+", "PE_18:2_22:0 [M+H]+", "PE_18:2_23:0 [M+H]+",
      "CER_15:0_18:1d7 [M+H]+",
      "CER_dihydroxy_16:0_26:1 [M+H]+", "CER_dihydroxy_18:0_24:0 [M+H]+", 
      "CER_hex_dihydroxy_18:1_26:0 [M+H]+", #???
      "DAG_15:0_18:1d7 [M+NH4]+", "DAG_16:0_16:0 [M+NH4]+", 
      "DAG_16:0_16:1 [M+NH4]+", "DAG_16:1_18:2 [M+NH4]+",
      "DAG_16:0_18:3 [M+NH4]+",
      "DAG_17:0_18:2 [M+NH4]+",
      "DAG_18:0_18:1 [M+C2H8N]+", "DAG_18:2_18:3 [M+NH4]+", 
      "DAG_18:2_20:0 [M+NH4]+", 
      "TAG_12:0_18:2_18:2 [M+NH4]+", 
      "TAG_14:0_16:0_18:2 [M+NH4]+", "TAG_14:0_16:0_18:3 [M+NH4]+", 
      "TAG_15:1_16:0_18:2 [M+NH4]+",
      "TAG_16:0_16:0_16:0 [M+NH4]+", "TAG_16:0_16:2_18:3 [M+NH4]+", 
      "TAG_16:0_17:0_18:2 [M+NH4]+", "TAG_16:0_17:1_18:2 [M+NH4]+",
      "TAG_17:2_18:2_18:3 [M+NH4]+",
      "TAG_18:1_18:2_22:0 [M+NH4]+", "TAG_18:2_18:2_18:2 [M+NH4]+", 
      "TAG_18:2_18:2_19:2 [M+NH4]+", "TAG_18:2_18:2_24:0 [M+NH4]+",
      "TAG_18:3_18:3_20:0 [M+NH4]+",
      "Sitosterol ester 18:3 [M+NH4]+",
      "Unk-010 _I [M+C2H8N]+",
      "Unk-010 _II [M+NH4]+", "Unk-010 _II [M+C2H8N]+", # ????????
      "Unk-010 _III [M+NH4]+", # ?????
      "Unk-010 _IV [M+NH4]+",
      "Unk-025 _18:1_18:2 [M+NH4]+",
      "Unk-026 _I [M+C2H8N]+", # ?????????
      "Unk-026 _II [M+C2H8N]+", "Unk-026 _III [M+NH4]+",
      "Unk-040 [M+C2H8N]+", 
      "Unk-168 [M+H]+", # ??????????
      "Unk-169 _18:3 [M+H]+", 
      "Unk-170 _I [M+H]+", "Unk-170 _II _18:2 [M+NH4]+", 
      "Unk-172 _18:3 [M+H]+", "Unk-176 [M+H]+", 
      "Unk-177 _16:0_18:0 [M+NH4]+", "Unk-178 _16:0_18:0 [M+H]+",
      "Unk-194 [M+H]+",
      "Unk-200 _18:2_18:3 [M+H]+", "Unk-206 _16:0_18:2_18:3 [M+H]+", 
      "Unk-210 _16:0 [M+H]+",
      "Unk-214 [M+H]+", "Unk-237 [M+H]+", "Unk-238 [M+H]+")
  }else if(s == "maturation" & p == "POS"){
    excl <- c(
      excl, 
      "PA_16:0_18:1 [M+NH4]+",
      "mPA_18:2_18:3 [M+C2H8N]+",
      "PC_16:0_16:1 [M+H]+",
      "PE_15:0_18:1d7 [M+H]+", "PE_16:0_18:1 [M+H]+",
      "PE_16:0_18:2 [M+H]+", 
      "PE_18:1_18:2 [M+H]+", "PE_18:2_23:0 [M+H]+", "PE_18:2_18:3 [M+H]+",
      "PE_18:2_22:0 [M+H]+",
      "PG_16:0_16:0 [M+NH4]+",
      "CER_hex_dihydroxy_18:1_26:0 [M+H]+", #???
      "DAG_15:0_18:1d7 [M+NH4]+","DAG_16:0_18:1 [M+C2H8N]+", 
      "DAG_16:0_18:3 [M+NH4]+", "DAG_18:2_18:3 [M+NH4]+",
      "TAG_14:0_16:0_18:1 [M+NH4]+","TAG_17:1_18:2_18:2 [M+NH4]+",
      "TAG_18:3_18:3_20:0 [M+NH4]+",
      "Sitosterol ester 18:3 [M+NH4]+",
      "Unk-089 [M+NH4]+", "Unk-169 _18:3 [M+H]+", "Unk-172 [M+H]+", 
      "Unk-176 [M+H]+",
      "Unk-178 [M+H]+", "Unk-200 _18:2_18:3 [M+H]+", 
      "Unk-206 _16:0_18:2_18:3 [M+H]+",
      "Unk-234 [M+H]+", "Unk-236 [M+H]+")
  }
