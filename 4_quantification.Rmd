...
for(i in seq(nrow(res))){
  ...
  if(cl %in% c(
    ...
    "LPG", "PG", "PI", "PS", "unk_A", "unk_B", "unk_C", "unk_D")){
    ad <- "[M-H]-"
    pol <- "NEG"
  } else if(cl %in% c(
    ...
    ad <- "[M+NH4]+"
    pol <- "POS"
  } else if(res$compound[i] %in% c(
    "(Epi)catechin", "Kaempferol glucoside",
    "Unk-006 (pph_glucoside)", "Unk-029 (18:2)", "Unk-032", "Unk-037 (14:3)",
    "Unk-039", "Unk-042", "Unk-043 (15:3)", "Unk-045 (I)", "Unk-075 (14:2/15:3)",
    "Unk-184 (glucoside)", "Unk-191 (16:0/18:1)", "Unk-202", "Unk-208", "Unk-211",
    ...
    "Unk-068", "Unk-079", "Unk-099", "Unk-173", "Unk-187", "Unk-198")){
    ad <- "[M+CHO2]-"
    pol <- "NEG"
  } else if(res$compound[i] %in% c(
    "Unk-001 (I)", "Unk-001 (II)", "Unk-003", "Unk-169 (18:3)", "Unk-172",
    "Unk-176", "Unk-178", "Unk-193", "Unk-194", "Unk-200 (18:2/18:3)",
    "Unk-206 (16:0/18:2/18:3)", "Unk-207", "Unk-209", "Unk-210 (16:0)", "Unk-214",
    "Unk-234", "Unk-235", "Unk-236", "Unk-237", "Unk-238")){
    ad <- "[M+H]+"
    pol <- "POS"
  } else if(res$compound[i] %in% c(
    "Sitosterol ester 18:3", 
    "Unk-005", "Unk-010 (I)", "Unk-026 (II)", "Unk-026 (III)", "Unk-040",
    "Unk-041 (18:2)", "Unk-051 (18:0/18:1/18:2)", "Unk-059 (II)", 
    "Unk-062 (16:0/18:2) (I)", "Unk-089")){
    ad <- "[M+NH4]+"
    pol <- "POS"
  }
  # to do: Malic acid
  if(exists("ad")){
    for(s in c("tissues", "maturation")){
      ...
      if(!is.na(cmps[cmps$compound == res$compound[i], paste0(
        substr(s, 1, 1), "_min")])){
        i.cmp[,"rtmin"] <- cmps[cmps$compound == res$compound[i], 
                                paste0(substr(s, 1, 1), "_min")]*60
      }
      if(!is.na(cmps[cmps$compound == res$compound[i], paste0(
        substr(s, 1, 1), "_max")])){
        i.cmp[,"rtmax"] <- cmps[cmps$compound == res$compound[i], 
                                paste0(substr(s, 1, 1), "_max")]*60
      }
      res[i, paste0("pw_", substr(s, 1, 1))] <- i.cmp[,"rtmax"] - i.cmp[,"rtmin"]
    ...
    #print(i)
    rm(ad, pol)
  } else{print(paste("Define adduct for compound", res$compound[i]))}
}
...
for(s in c("tissues", "maturation")){
  ...
  CV <- (apply(data[idx,], 2, sd, na.rm = TRUE) / apply(
    data[idx,], 2, mean, na.rm = TRUE))*100
  ...
  if(s == "tissues"){
    idx <- grep("solv", rownames(data))
  } else if(s == "maturation"){
    idx <- grep("slv", rownames(data))
  }
  m_slv <- colMeans(data[idx,])
  m_slv[m_slv == 0] <- 1
  idx <- grep("QC", rownames(data))
  idx <- idx[!grepl("rw|dl", rownames(data)[idx])]
  m_qc <- colMeans(data[idx, ])
  assign(paste0("FC_", s), m_qc/m_slv)
}
...
```{r output}
res <- as.data.frame(cbind(res$compound, 
                           CV_tissues, lin_tissues, FC_tissues,
                           CV_maturation, lin_maturation, FC_maturation))
colnames(res) <- c("compound", "t_CV", "t_linearity", "t_FC",
                   "m_CV", "m_linearity", "m_FC")
res$t_CV <- as.numeric(sprintf("%.1f", as.numeric(res$t_CV)))
res$m_CV <-  as.numeric(sprintf("%.1f", as.numeric(res$m_CV)))
res$t_linearity <-  as.numeric(sprintf("%.3f", as.numeric(res$t_linearity)))
res$m_linearity <-  as.numeric(sprintf("%.3f", as.numeric(res$m_linearity)))
res$t_FC <-  as.numeric(sprintf("%.1f", as.numeric(res$t_FC)))
res$m_FC <-  as.numeric(sprintf("%.1f", as.numeric(res$m_FC)))
