...
for(i in seq(nrow(cmps))){
  ...
  if(!(
    ...
    (cmps$name[i] %in% c(
      ...
      "HexCer;O4 45:1", "MGDG 16:1_18:3", "MGDG 16:3_18:2", "MGDG 36:0",
      "MGDG 18:0_18:3", "DGDG 34:0", "DGDG 34:2", "DGDG 16:0_18:3", 
      "DGDG 18:1_18:2", "DG 18:0_18:1", "TG 14:0_14:0_14:0", "TG 49:2", 
    ...
    for(p in c("POS", "NEG")){
      ...
      if((!paste(p, cmps$name[i]) %in% c(
      ...
      )) & (!paste(p, cmps$class[i]) %in% c("NEG TG", "NEG Glc-ST"))){
        if(p == "POS"){
          tmp_ionsdb <- ionsdb[
        ...
        if(p == "POS" & cmps$class[i] %in% c(
          ...
          "DGMG", "MGDG", "DGDG", "SQDG", "acMGDG", "MG", "DG", "TG", "ST", "Glc-ST")){
          tmp_add <- "[M+NH4]+"
        ...
        if(length(i_spectra) > 0){
          ...
          if((paste(p,cmps$class[i]) %in% c("POS PC")) |
             (paste(p, cmps$name[i]) %in% c("POS MGDG 18:1_18:2"))){
            ms2comb <- filterMzValues(ms2comb, 283.7007, 
                                      ppm = 20, keep = FALSE)
          }
          ...
          if(cmps$class[i] %in% c("PC")){
            xmin <- 400
          } else if(cmps$class[i] %in% c("TG", "ST")){
            xmin <- 300
          } else if(cmps$class[i] %in% c(
            "PA", "mPA", "dmPA", "PE", "PG", "PI", "MGDG", "SQDG")){
            xmin <- 200
