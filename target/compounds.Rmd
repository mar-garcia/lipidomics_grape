...
data <- merge(cmps, fmls, by = "ID", all = TRUE)
rm(fmls, IS, cmps)
...
data$NH4 <- unlist(MetaboCoreUtils::mass2mz(data$mass, "[M+NH4]+"))
...
DT::datatable(data[,c(#"RT_min", "RT_max", 
  "class", "ID", "formula", "NH4", "RTx")], rownames = FALSE)
```
