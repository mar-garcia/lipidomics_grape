...
<span style="color:red">**Should we reduce the threshold on `dev` parameter to 0.6 or even 0.57?**</span>  
...
Then, we compute a distance matrix for the generated data with the function `TSDatabaseDistances` of the R package `TSdist`. In addition to use logarithmic data, for this estimation the values are also scaled.   
Finally, we do a new clustered heatmap to ascertain the possible existence of compound groups (i.e., clusters) showing similar trends along the time.  
...
dt_pred_dist <- TSdist::TSDatabaseDistances(
  dt_pred_s_t, distance = "tquest", 
  tau = mean(dt_pred_s_t), diag = T, upper = T)

km_tq <- data.frame(
  cluster = KMedoids(as.matrix(dt_pred_dist), 2, "ccor" #"tquest", tau = 0
                     ))
km_tq$cluster <- paste0("c", km_tq$cluster)
rownames(km_tq) <- rownames(dt_pred_s_t)

pheatmap(dt_pred_dist, show_colnames = F, show_rownames = F,
         color = colorRampPalette(brewer.pal(n = 7, name = "RdYlBu"))(100),
         #cutree_rows = 5, cutree_cols = 5, 
         annotation_row = km_tq, annotation_col = km_tq, legend = FALSE,
         main = "Clustering of distance measurements on predicted data")
```

From the heatmap output we can assume the existence of 5 groups of compounds with different trends over time.  
<span style="color:red">**We still have to discuss if we are happy with these 5 clusters or if we want to change the number of clusters.**</span>  
...
pm_cl <- cutree(pm$tree_row, k = 5)
data_st_log <- t(log10(data_st[padj < 0.05 & dev > 0.7,]))
...
for(j in seq(5)){
  ...
  points(ytimes, dt_pred_s_t[idx[i],], pch = 16, col = clrs[i], type = "l", lwd = 2)
  for(i in 2:length(idx)){
    points(xtime, data_st_log_s[,idx[i]], pch = 16, col = alpha(clrs[i], 0.6))
    points(ytimes, dt_pred_s_t[idx[i],], col = clrs[i], pch = 16, type = "l", lwd = 2)
  }
...
all(rownames(pm_cl) == nodes$ID) # TRUE
nodes$cluster <- pm_cl
...
all(cmps$ID[idx] == names(pm_cl)) # TRUE
cmps$cluster <- "NS"
cmps$cluster[padj < 0.05 & dev < 0.7] <- "high-dev"
cmps$cluster[idx] <- paste0("cl", pm_cl)
...
dt_cl <- data.frame(cluster = paste0("cl", pm_cl))
...
pheatmap(dt, annotation_row = dt_pt2, annotation_col = dt_cl,
         annotation_colors = ann_color, show_colnames = F, show_rownames = F)
```


```{r plots, eval = TRUE}
cmps <- cmps[order(cmps$cluster), ]
...
for(i in seq(nrow(cmps))){
  ...
  p[[i]] <- ggplot(Sample_data, aes(x, log10(y))) + 
    geom_point(color = colorRampPalette(brewer.pal(9, "YlOrRd"))(13)[as.factor(Sample_data$class)]) + 
    geom_smooth(method = "gam", formula = y ~s(x), col = "grey", alpha = 0.1) +
    xlab("") + ylab("") + theme_light() + 
    ggtitle(ttl <- paste0(cmps$ID[i], " (", cmps$cluster[i], ")"))
}
ggsave(
