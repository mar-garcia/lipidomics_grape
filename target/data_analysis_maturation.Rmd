...
gam.cor.pred <- function(x){
  cor(log10(x), 
      predict.gam(gam(log10(x) ~ s(xtime), method = "REML"), 
...
for(i in seq(ncol(dt_pred))){
  dt_pred[,i] <- predict.gam(gam(log10(dt[,idx[i]]) ~ s(xtime), method = "REML"), 
                             data.frame(xtime = ytimes))  
}


cormat <- cor(dt_pred)
# pairwise correlations between predicted values by each model:
pm <- pheatmap(cormat, show_colnames = F, show_rownames = F)
```

This classification is based on predicted values, following this scheme:  
  
  - predicted values are calculated within the time range of the present study  
  - eight of the predicted values are taken on a time-equidistant basis  
  - the angle between every 2 consecutive points is calculated  
  - an increase or decrease is considered if the angle is >10°C (otherwise it is considered "flat")  

```{r trends}
trends <- c()
xx <- seq(length(ytimes))
xx2 <- xx[seq(1, 50, 7)]
for(i in seq(ncol(dt))){
  ...
  i.pred <- predict.gam(gam_i, data.frame(xtime = ytimes))
  i.pred_antilog <- 10^i.pred
  i.pred.norm <- (i.pred_antilog - min(i.pred_antilog)) / 
    (max(i.pred_antilog) - min(i.pred_antilog))
  yy <- i.pred.norm*length(ytimes)
  yy2 <- yy[seq(1, 50, 7)]
  tmp <- data.frame(
    direction = diff(i.pred.norm[seq(1, 50, 7)]),
    slope = atan(diff(yy2) / diff(xx2))*(180/pi)
  )
  #deltas <- diff(i.pred.norm)
  #tmp <- data.frame(
  #  direction = deltas, 
  #  slope = deltas/mean(deltas)
  #)
  tmp$tendency <- "flat"
  tmp$tendency[abs(tmp$slope) > 10 & tmp$direction < 0] <- "down"
  tmp$tendency[abs(tmp$slope) > 10 & tmp$direction > 0] <- "up"
...
res$trends4[!res$trends4 %in% c(
  "down", "down_flat", "down_flat_up", "down_up",
  "up", "up_flat", "up_flat_down", "up_down" 
)] <- "others"
res$trends4 <- factor(res$trends4, levels = c(
  "up", "up_flat", "up_flat_down", "up_down",
  "down", "down_flat", "down_flat_up", "down_up",
  "others"))
table(res$trends4)
...
And here with the predicted values:

```{r pca-predicted}
dt <- data.frame(scaling.pareto(dt_pred))
pca_sig <- prcomp(dt, center = FALSE, scale. = FALSE)
tmp <- data.frame(pca_sig$x)
plot_ly(x = tmp$PC1, y = tmp$PC2,
        text = zoo::as.Date(ytimes))
