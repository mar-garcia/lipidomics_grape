...
fl <- "C:/Users/lenovo/Documents/GitHub/lipidomics/new_rt.xlsx"
...
ID <- colnames(dt)
dt.ma <- cbind(ID, Time, Subject, t(dt))
dt.ma[is.na(dt.ma)] <- 0
dt.ma <- dt.ma[as.numeric(dt.ma[,"Time"]) <= 11, ]
write.csv(dt.ma, "data/dt_metaboanalyst.csv", row.names = FALSE)
...
pval.ma <- data.frame(mSet$analSet$aov2$sig.mat)
#tmp2 <- data.frame(mSet$analSet$MB$stats)
pval.ma <- pval.ma$Raw.P.val
padj.ma <- p.adjust(pval.ma, "BH")
```
...
draw.triple.venn(
  area1 = sum(pval.lm < 0.05),
  area2 = sum(pval.li < 0.05),
  area3 = sum(pval.ma < 0.05),
  n12 = sum(pval.lm < 0.05 & pval.li < 0.05),
  n13 = sum(pval.lm < 0.05 & pval.ma < 0.05),
  n23 = sum(pval.li < 0.05 & pval.lm < 0.05),
  n123 = sum(pval.lm < 0.05 & pval.li < 0.05 & pval.ma < 0.05),
  category = c("Regression", "limma", "MetaboAnalyst"),
...
draw.triple.venn(
  area1 = sum(padj.lm < 0.05),
  area2 = sum(padj.li < 0.05),
  area3 = sum(padj.ma < 0.05),
  n12 = sum(padj.lm < 0.05 & padj.li < 0.05),
  n13 = sum(padj.lm < 0.05 & padj.ma < 0.05),
  n23 = sum(padj.li < 0.05 & padj.ma < 0.05),
  n123 = sum(padj.lm < 0.05 & padj.li < 0.05 & padj.ma < 0.05),
  category = c("Regression", "limma", "MetaboAnalyst"),
  fill = c("#E69F00", "#56B4E9", "#009E73")
)
```
...
```{r}
idx <- which(padj.lm < 0.05 & padj.li > 0.05 & padj.ma > 0.05)
p <- list()
...
```{r}
idx <- which(padj.lm > 0.05 & padj.li < 0.05 & padj.ma > 0.05)
idx <- idx[sample(length(idx), 15*3)]
idx <- idx[order(idx)]
p <- list()
for(i in seq(length(idx))){
  tmp_dt <- data.frame(
...
Here I plot a random subset of compounds significant only by the methods limma-MetaboAnalyst.
