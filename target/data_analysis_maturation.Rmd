...
Then, we compute a distance matrix for the generated data with the function `TSDatabaseDistances` of the R package `TSdist`. As a distance measure we use the algorithm `dtw` (i.e., *dynamic time warping*). In addition to use logarithmic data, for this estimation the values are also scaled.   
Finally, clustering based on slope/shape/trend similarity over time is done using the algorithm `pacf` (i.e., *partial autocorrelation function*).
...
dt_pred_dist <- TSdist::TSDatabaseDistances(
  dt_pred_s_t, distance = "dtw", diag = T, upper = T)

km_tq <- data.frame(cluster = KMedoids(as.matrix(dt_pred_dist), 6, "pacf"))
km_tq$cluster <- paste0("c", km_tq$cluster)
rownames(km_tq) <- rownames(dt_pred_s_t)

pm <- pheatmap(dt_pred_dist, show_colnames = F, show_rownames = F,
...
pheatmap(as.matrix(dt_pred_dist)[order(km_tq$cluster, pm$tree_row$order),
                                 order(km_tq$cluster, pm$tree_row$order)], 
...
for(j in seq(6)){
  idx <- which(km_tq$cluster == paste0("c", j))
  if(length(idx) > 9){
...
all(rownames(km_tq) == nodes$ID) # TRUE
nodes$cluster <- km_tq$cluster
...
all(cmps$ID[idx] == names(km_tq$cluster)) # TRUE
cmps$cluster <- "NS"
cmps$cluster[padj < 0.05 & dev < 0.7] <- "high-dev"
cmps$cluster[idx] <- km_tq$cluster
...
dt_cl <- data.frame(cluster = km_tq$cluster)
