...
indep <- "brix" # specify "brix" or "date"
library(readxl)
library(xcms)
library(plotly)
library(knitr)
...
# Delete compounds not full-filling KMD criteria
excl <- read_xlsx("data/MRMeliminati_dopoKMD.xlsx", col_names = FALSE)
colnames(excl) <- "ID"
excl$ID <- gsub("TAG\\(", "TAG", excl$ID)
excl$ID <- gsub("-FA", "_FA", excl$ID)
idx <- grep("TAG", excl$ID)
excl$ID[idx] <- gsub(")", "", excl$ID[idx])
table(excl$ID %in% data$ID)
data <- data[!data$ID %in% excl$ID, ]
...
cmps <- read.csv("C:/Users/garciaalom/Google Drive/projectes/lipidomics/target/lipidomics_shared/compounds_target.csv")
...
## Non validated compounds

```{r non-validated-cmps}
valid <- read_excel("C:/Users/garciaalom/Google Drive/projectes/lipidomics/target/lipidomics_shared/target/composti_grape_application_validati.xlsx",
                    col_names = FALSE)
colnames(valid) <- "ID"
valid$ID <- gsub("[^0-9A-Za-z///' ]", "_", valid$ID)
valid$ID <- gsub("/", "_", valid$ID)
valid$ID <- gsub(" ", "_", valid$ID)
valid$ID <- gsub("__", "_", valid$ID)
idx <- grep("_$", valid$ID)
valid$ID[idx] <- substr(valid$ID[idx], 1, nchar(valid$ID[idx])-1)
data <- data[rownames(data) %in% valid$ID, ]
cmps <- cmps[cmps$ID %in% valid$ID, ]
dim(data)
```
...
[**We still have to decide if to exclude some compounds based what it has been observed within QC samples.**]{style="color:red"}
...
```{r pca-score, fig.height=6, fig.width=8}
...
plot_ly(x = tmp$PC1, y = tmp$PC2,
        text = rownames(tmp),
        color = cmps$class)
rm(tmp, dt, class2, idx, IS, scaling.pareto)
```

## Manuscript

```{r pca-paper, fig.width=10}
class_col <- c(colorRampPalette(brewer.pal(10, "PiYG"))(13), "grey")
names(class_col) <- unique(class)

par(mfrow = c(1,2), mar = c(5,4,4,0.5))
tmp <- data.frame(pca$x)
plot(tmp$PC1, tmp$PC2, axes = FALSE,
     xlab = paste0("PC1 (", round(summary(pca)$importance[2,1]*100), "%)"),
     ylab = paste0("PC2 (", round(summary(pca)$importance[2,2]*100), "%)"),
     col =  class_col[class], ylim = c(-6.8, 8), 
     pch = 16, main = "Score plot")
grid()
abline(v=0, h=0, col = "grey", lty = 2)
tmp2 <- gsub("Pt_0", "", class)
tmp2 <- gsub("QC", "", tmp2)
tmp2 <- gsub("Pt_", "", tmp2)
points(tmp$PC1, tmp$PC2, col = class_col[class], pch = 16) 
text(tmp$PC1, tmp$PC2, tmp2, pos = 1, cex = 0.6)
axis(1, at = seq(-30, 30, by = 10), col = NA)
axis(2, at = seq(-20, 20, by = 5), col = NA)
...
col_class <- c(col_class[14], col_class[13], col_class[15], col_class[16], col_class[8], col_class[2], col_class[17], col_class[3], col_class[9], col_class[5], col_class[6], col_class[18], col_class[19], col_class[7], col_class[1], col_class[10], col_class[4], col_class[11], col_class[12], col_class[20], col_class[21])
cmps$class <- gsub("Lyso_", "L", cmps$class)
cmps$class <- gsub("FFA", "FA", cmps$class)
cmps$class <- gsub("MAG", "MG", cmps$class)
cmps$class <- gsub("DAG", "DG", cmps$class)
cmps$class <- gsub("TAG", "TG", cmps$class)
col_class <- col_class[names(col_class) %in% cmps$class]

tmp <- data.frame(pca$rotation)
plot(tmp$PC1, tmp$PC2, axes = FALSE, xlab = "PC1", ylab = "PC2",
     col =  col_class[cmps$class], xlim = c(-0.05, 0.15), 
     pch = 16, main = "Loading plot")
grid()
abline(v=0, h=0, col = "grey", lty = 2)
axis(1, at = seq(-1, 1, by = 0.02), col = NA)
axis(2, at = seq(-1, 1, by = 0.02), col = NA)
add_legend("right", legend = names(col_class), pch = 16, 
           col = col_class, horiz = FALSE, cex = 0.75)
rm(pca)
```
...
```{r pls-val, fig.width = 4, eval=TRUE}
...
boxplot(c(R2.vals, R2.rndm) ~ c(rep(1, nsubmod), rep(2, nsubmod)), 
        col = "white", ylim = c(-1, 1), ylab = expression("Q"^2), 
...
```{r pls-val-hm}
col_class_cmps <- c(
  Lyso_PC = "#E41A1C", DG = "#974661", PG = "#4A72A6", 
  Lyso_PG = "#3E8E93", PC = "#48A462", MGDG = "#5D995D", Lyso_PA = "#7E6E85", 
  Lyso_PI = "#A35390", PS = "#D16948", PE = "#FF7F00", DGDG = "#FFB716", 
  CAR = "#FFF02D", MG = "#E1C62F", CER = "#B97B2A", FA = "#B75F49", 
  Lyso_PE = "#DB728C", PI = "#EC83BA", PA = "#C28EA9", TG = "#999999")
...
aggregate(tmp$RC, by = list(tmp$quantile), range)
...
tmp %>% 
  ggplot() +
  geom_histogram(aes(x = RC), fill = "steelblue", col = "white") + 
  xlab("Regression Coefficients") + 
  geom_vline(xintercept = quantile(tmp$RC,0.25), col = "red", lty = 2) + 
  geom_vline(xintercept = quantile(tmp$RC,0.75), col = "red", lty = 2) + 
  theme_light() + 
...
rownames(tmp2) <- gsub("Lyso_", "L", rownames(tmp2))
rownames(tmp2) <- gsub("FFA", "FA", rownames(tmp2))
rownames(tmp2) <- gsub("MAG", "MG", rownames(tmp2))
rownames(tmp2) <- gsub("DAG", "DG", rownames(tmp2))
rownames(tmp2) <- gsub("TAG", "TG", rownames(tmp2))
pheatmap(tmp2[,-2], cluster_cols = F, cutree_rows = 1, 
         color = colorRampPalette(brewer.pal(n = 9, name ="Blues"))(100),
         cellwidth = 50, fontsize = 8)
pheatmap(tmp2[,-2], cluster_cols = F, cutree_rows = 4, 
         color = colorRampPalette(brewer.pal(n = 9, name ="Blues"))(100),
         cellwidth = 50, fontsize = 8)
```

**When using `brix degree` as independent variable:** Heatmap clearly shows that there is a group of compound classes (`CAR`, `FFA`, `TAG`, `MAG`, `DAG` and `PS`) for which most of compounds do not have a strong regression coefficient, that is, in general do not seem to be sensitive to ripening.\
...
plt.cls <- function(cls, clr, dcrs){
  ...
  if(indep == "brix"){
    plot(brix$Brix, scale(dt[,cmps$ID[idx][j]]), xlab = "", ylab = "", 
         main = gsub("Lyso_", "L", cls), bty = "l", 
         col = pal(length(idx))[i], 
         ylim = c(min(scale(dt[,cmps$ID[idx]])), max(scale(dt[,cmps$ID[idx]]))))
    abline(lm(scale(dt[,cmps$ID[idx][j]])~brix$Brix), 
           col = pal(length(idx))[i])
    title(xlab = "°Brix", line = 2, cex.lab = 1.2, ylab = "scaled value")
    
    for(i in 2:length(idx)){
      j <- order(tmp$RC[idx], decreasing = dcrs)[i]
      points(brix$Brix, scale(dt[,cmps$ID[idx][j]]), 
             col = pal(length(idx))[i])
      abline(lm(scale(dt[,cmps$ID[idx][j]])~brix$Brix), 
             col = pal(length(idx))[i])
    }
    ...
    plot(xtime, scale(dt[,cmps$ID[idx][j]]), xlab = "Brix", ylab = "", 
         ...
         col = pal(length(idx))[i], 
         ylim = c(min(scale(dt[,cmps$ID[idx]])), max(scale(dt[,cmps$ID[idx]]))))
    abline(lm(scale(dt[,cmps$ID[idx][j]])~xtime), 
           col = pal(length(idx))[i])
    for(i in 2:length(idx)){
      j <- order(tmp$RC[idx], decreasing = dcrs)[i]
      points(xtime, scale(dt[,cmps$ID[idx][j]]), 
             col = pal(length(idx))[i])
      abline(lm(scale(dt[,cmps$ID[idx][j]])~xtime), 
             col = pal(length(idx))[i])
    }
...
```{r plots-beh}
par(mfrow = c(2, 3), mar = c(4, 3, 2, 0.5))
#for(i in seq(length(unique(cmps$class)))){
#  print(plt.cls(cls = unique(cmps$class)[i]))
#}
plt.cls(cls = "PE", clr = "Greens", dcrs = F)
plt.cls(cls = "PG", clr = "Greens", dcrs = F)
plt.cls(cls = "LPC", clr = "Greens", dcrs = F)

plt.cls(cls = "LPE", clr = "Greens", dcrs = F)
plt.cls(cls = "CER", clr = "Reds", dcrs = T)
plt.cls(cls = "DGDG", clr = "Reds", dcrs = T)
```


```{r plots-beh2, fig.height=7, fig.width=5}
...
```{r}
rownames(data)
write.csv(data.frame(cmps = rownames(data)), "stats_maturation_compounds.csv",
          row.names = FALSE)
```
