...
for(i in 1:(length(cmps.class))){
  idx <- which(cmps$class == cmps.class[i])
  if(length(idx) == 1){
    print(paste(cmps.class[i], 
                cmps$ID[idx], 
                round(max(tmp_dt[idx]), 1), 
                sep = " - "))
  } else {
    tmp <- apply(tmp_dt[cmps$ID[idx],], 1, max)
    print(paste(cmps.class[i], 
                cmps$ID[idx][which.max(tmp)], 
                round(max(tmp), 1), 
                sep = " - "))
  }
}
...
```{r filter-data}
class_st <- class[class != "QC" & !grepl("MIX", colnames(data))]

data_st <- data[, (class != "QC" & !grepl("MIX", colnames(data)))]
...
IS <- gsub("_Na", "", IS)
data_st <- data_st[!rownames(data_st) %in% IS,]
...
```{r gam, warning = TRUE, message = TRUE}
stat.gam <- function(x){
...
```{r gam2, warning = TRUE, message = TRUE}
...
```{r gam-cor-pred, warning = TRUE, message = TRUE}
stat.dev <- function(x){
  summary(
    gam(x ~ s(xtime), method = "REML", gamma = 1.4, family = Gamma(link = "log")
        ))$dev.expl 
...
<span style="color:red">**Should we modify the threshold on `dev` parameter?**</span>  
...
plot_ly(x = tmp$PC1, y = tmp$PC2,
        text = rownames(tmp),
        color = class2[class != "QC" & !grepl("MIX", colnames(data))])
...
for(i in seq(ncol(dt_pred))){
  dt_pred[,i] <- predict.gam(
    gam(dt[,idx[i]] ~ s(xtime), method = "REML", gamma = 1.4, 
...
all(names(padj) == cmps$ID) # TRUE

idx <- which(padj < 0.05 & dev > 0.7)

dt <- data_st[idx,]
set.seed(123)
dt <- t(imputeRowMinRand(as.matrix(dt), method = "from_to",
                         min_fraction = 1/100,
                         min_fraction_from = 1/1000))

# Calculate predicted values according to significant models
...
# Calculate the distance matrix of compounds time-trends
distmat <- as.matrix(TSclust::diss(t(scale(dt_pred)), METHOD = "COR"))

# Multidimensional scaling on distance matrix: https://www.statmethods.net/advstats/mds.html
...
This plot is a bidimensional representation of the distances on mean(time-trends). 
It shows the variability in the time evolution of measured compounds during ripening.   

In the graph we can clearly see 2 groups, one being more homogeneous than the other.  
Below we plot 3 examples of compounds from the homogeneous group (i.e., the one with the highest value in 2n dimension, the one with the closes value to 0 on the 2n dimension, and the one with the smallest value on the 2n dimension):


```{r}
mycmps <- c(names(which.max(fit$points[,2][fit$points[,1] > 0])),
            names(which.min(fit$points[,2][fit$points[,1] > 0])),
            names(which.min(abs(median(fit$points[,2][fit$points[,1] > 0]) - fit$points[,2][fit$points[,1] > 0]))))
p <- list()
x <- as.integer(as.Date(brix$data))
for(i in seq(length(mycmps))){
  y <- dt[,mycmps[i]]
  z <- is.na(data[mycmps[i], class != "QC" & !grepl("MIX", colnames(data))])
  Sample_data <- data.frame(y,x, z)
  Sample_data$class <- class[class != "QC" & !grepl("MIX", colnames(data))]
...
We see that the homogeneous group is characterised by increasing concentrations of the compounds over time. This may indicate that the accumulation processes tend to present a more homogeneous trend.  

Now we will plot 3 examples of compounds from the other "cluster" (i.e., the one with the highest "intra-group" variability):   

```{r}
mycmps <- c(names(which.max(fit$points[,2][fit$points[,1] < 0])),
            names(which.min(fit$points[,2][fit$points[,1] < 0])),
            names(which.min(abs(median(fit$points[,2][fit$points[,1] < 0]) -
                                  fit$points[,2][fit$points[,1] < 0]))))
p <- list()
x <- as.integer(as.Date(brix$data))
for(i in seq(length(mycmps))){
  y <- dt[,mycmps[i]]
...
plot_ly(x = tmp$PC1, y = tmp$PC2,
        text = rownames(tmp),
        color = class2[class2 != "QC" & !grepl("MIX", colnames(data))])
