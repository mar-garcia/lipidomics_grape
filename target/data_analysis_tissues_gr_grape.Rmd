...
Proportion of quantified lipids by gr of grape:
...
Total of quantified lipids by 2 mg of grape (it is not proportional!!! It indicates the total amount of quantified lipids and the amount of “other” compounds is obtained by extracting the calculated lipid value from 2 mg).

```{r}
par(mfrow = c(2, 2), mar = c(0, 5, 1, 4))
for(i in seq(nrow(quanti))){
  slices <- c(quanti$x[i], 2e3 - quanti$x[i]) 
  lbls <- c("lipids", "others")
  lbls <- paste0(lbls, "\n(", round(slices/1000), 'mg/2mg)')
  pie(slices, lbls, main = quanti$Group.1[i])
}
```
...
for(j in seq(length(classx))){
  ...
  slices <- quanti$x
  lbls <- quanti$Group.1
  pie(slices, lbls, main = classx[j], col = col_class_cmps[lbls])
}
```
