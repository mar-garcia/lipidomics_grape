...
data_n9 <- data_n9[!rownames(data_n9) %in% IS,]
...
```{r}
par(mfrow = c(2, 2), mar = c(2, 3, 1, 0.5))
for(j in seq(length(classx))){
  tmp <- data[, class == classx[j]]
  tmp$mean <- rowMeans(tmp)
  quanti <- aggregate(tmp$mean, list(cmps$class), function(x){sum(x, na.rm=T)})
  barplot(quanti$x, horiz = T, main = classx[j],
          col = col_class_cmps[quanti$Group.1],
          names = quanti$Group.1, las = 1, cex.names = 0.6)
}
```
...
Proportion of distribution for characteristic compounds:
...
Proportion of distribution for characteristic compounds considering the tissue with the highest values (those compounds for which there where >1 category as the one with the highest values are excluded):
...
Proportion of characteristic compounds by class:
...
Proportion of characteristic compounds per compound class:
...
Proportion of characteristic compound classes by tissue:
