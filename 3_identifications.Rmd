...
#cmp <- cmp[!cmp %in% c("PE_16:0_18:1 _I", "PE_16:0_18:1 _II", "PS_18:2_18:2 _II")]
...
datatable(res[,-c((ncol(res)-3):ncol(res))], #rownames = FALSE, 
          options = list(
...
for(o in seq(n)){
  ...
  for(i in seq(q, q+7)){
    if(i <= length(cl)){
      ...
      points(x, dx$KMD_H, pch = 19, col = mycols[as.character(dx$db)], cex = 2)
      idx <- which(duplicated(paste0(" ", dx$C, ":", dx$db, " ")))
      if(length(idx) > 0){
        text(x[-idx], dx$KMD_H[-idx], paste0(" ", dx$C, ":", dx$db, " ")[-idx], 
             pos = 1, offset = 1)
      } else {
        text(x, dx$KMD_H, paste0(" ", dx$C, ":", dx$db, " "), 
             pos = 1, offset = 1)
      }
