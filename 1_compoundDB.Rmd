...
Define a function that allows to calculate molecular formulas according to each lipid class structure/backbone and the total number of carbons and double bonds of their fatty acyl chains.
...
Generate all potential lipid species build with fatty acyl chains with a number of carbons between 12 and 24, and with up to 6 unsaturations, except for sphingolipids for which the potential number of carbons of fatty acyl chains is extended until 28.

```{r}
cmps <- c()

cls <- c("FFA", "CAR", "LPC", "LPI", "MG")
...
cls <- c("PA", "PC", "PE", "PG", "PI", "PS", "MGDG", "DGDG", "DG")
...
cmps_RT <- cmps_RT[!cmps_RT$compound %in% c(
  #"FFA(22:0)", 
  "PA 18:1_18:3"#, "PC(18:1/18:3)", "PE(18:1/18:1)", "PE(18:1/18:3)", "PG(18:1/18:3)"
  ),]
...
for(i in seq(nrow(cmps))){
  ...
  if(length(idx) > 1){
    cmps <- rbind(cmps, cmps[i,])
    cmps$compound_id[nrow(cmps)] <- paste0(
      "C", as.numeric(gsub("C", "", cmps$compound_id[nrow(cmps)-1])) + 1)
    cmps$name[c(i, nrow(cmps))] <- cmps_RT$compound[idx]
    cmps$rtime[c(i, nrow(cmps))] <- cmps_RT$rtime[idx]
    cmps$std[c(i, nrow(cmps))] <- cmps_RT$standard[idx]
...
cmps <- rbind(cmps, 
              data.frame(
                compound_id = paste0("C", as.numeric(gsub("C", "", cmps$compound_id[nrow(cmps)])) + seq(nrow(cmps_RT_IS))),
...
for(i in seq(nrow(cmps))){
  if(cmps$instd[i] == "TRUE"){
    ...
    cmps$exactmass[i] <- MonoisotopicMass(
      formula = ListFormula(cmps$formula[i]))
  }
...
```{r}
metad <- make_metadata(source = "lipcmps", url = "",
                       source_version = "2", 
