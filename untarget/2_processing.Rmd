...
table(ms2$intensity_max > int)

ms2$good <- FALSE
ms2$good[ms2$mz_max < precursorMz(ms2) &
           ms2$intensity_max > int] <- TRUE
table(ms2$good)
ms2 <- ms2[ms2$good]
```
...
for(i in seq(nrow(ft))){
  idx_ms2 <- unlist(matchWithPpm(ft$mzmed[i], precursorMz(ms2), ppm = 5))
  idx_cp <- unlist(matchWithPpm(ft$mzmed[i], cp[,"mz"], ppm = 5))
  cpx <- cp[idx_cp, ]
  if(length(idx_ms2) > 0){
    for(j in seq(length(idx_ms2))){
      ...
      if((length(idx_cp) == 1 & cpx["sample"] == idy) |
        ...
        if((rtime(ms2)[idx_ms2][j] > rtmin) & (rtime(ms2)[idx_ms2][j] < rtmax)){
          ms2$FT[idx_ms2][j] <- rownames(ft)[i]
        }
      }
    } # close idx "j"
  }
...
table(ms2$FT != "X")
ms2 <- ms2[ms2$FT != "X"]
length(unique(ms2$FT))
ft <- ft[rownames(ft) %in% unique(ms2$FT),]
...
tmp2 <- paste(round(tmp$precursor), round(tmp$rtime))
...
ms2sim <- function(compound, precursor, rtime){
  idx <- which(db_thr$id %in% unlist(strsplit(compound, " - ")))
  idx <- idx[unlist(matchWithPpm(
    precursor, 
    db_thr[idx, paste0(tolower(polarity), "_precursor")], ppm = 5))]
  if(length(idx) > 0){
    ms2sub <- filterPrecursorMz(ms2, precursor + 0.01 * c(-1, 1))
    ms2sub <- filterRt(ms2sub, rtime + 10 * c(-1, 1))
...
db_exp <- db_exp %>%
  ...
  mutate(sim = pmap(list(compound, precursor, rtime),
                    function(compound, precursor, rtime){
                      ms2sim(compound, precursor, rtime)
                    }))
