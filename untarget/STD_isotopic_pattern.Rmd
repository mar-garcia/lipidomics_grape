...
for(pol in c("POS", "NEG")){
  ...
  if(any(duplicated(tmp2))){
    ...
    for(i in unique(dpl)){
      ...
      #                                           collapse = " - ")
      i.tmp <- i.tmp[which.min(abs(i.tmp$rt - i.tmp$target_rtime)),]
      #}
    }
  }
  ...
  dt_cmps <- data.frame(matrix(ncol = 9, nrow = length(cmpsx)))
  colnames(dt_cmps) <- c("cmp", "formula", "adduct", "i1d", "i2d", "i3d", "ips", 
                         "others", "ips_others")
  dt_cmps$cmp <- cmpsx
  
  for(i in seq(length(cmpsx))){
    ...
    dt_cmps$ips[i] <- median((1-(abs(iso_thr[1,2] - iso_exp_r$i1) + 
    ...
    myform <- myform[myform$H <= (2*myform$C + myform$N + 2),]
    myform$exactmass <- NA
    myform$NM <- NA
    for(j in seq(nrow(myform))){
      myform$exactmass[j] <- getMolecule(myform$formula[j])$exactmass
      myformx <- countElements(myform$formula[j])
      pippo <- ref[names(myformx)]
      myform$NM[j] <- t(pippo) %*% myformx
    }
    myform <- myform[myform$N %% 2 == myform$NM %% 2,]
    ...
    if(length(myform) > 0){
      ...
      for(j in seq(length(myform))){
        ...
        dev[j] <- median((1-(abs(iso_thrx[1,2] - iso_exp_r$i1) + 
                               abs(iso_thrx[2,2] - iso_exp_r$i2)  + 
                               abs(iso_thrx[3,2] - iso_exp_r$i3)))*100)
      }
      dt_cmps$ips_others[i] <- paste(round(dev[order(dev)]), collapse = ", ")
      ips_others <- c(ips_others, dev)
      ...
      dt_cmps$ips_others[i] <- "-"
    }
  }
  
  par(mfrow = c(1, 3))
  for(j in seq(3)){
    print(plot(dt_cmps$ips[seq(i)], 
               dt_cmps[seq(i), paste0("i", j, "d")]*100, 
               xlab = "Isotopic Pattern Similarity (%)", 
               ylab = "Isotope Deviance (%)", 
               main = paste0("Isotope ", j, 
                             "\ncor = ", sprintf(
                               "%.3f", cor(
                                 dt_cmps$ips[seq(i)], 
                                 dt_cmps[seq(i), paste0("i", j, "d")]*100
                               )))))
    abline(lm(dt_cmps[seq(i), paste0("i", j, "d")]*100 ~ 
                dt_cmps$ips[seq(i)]), col = "grey", lty = 2)
  }
  par(mfrow = c(1, 1))
  hist(ips_others, 
       breaks = seq(min(c(dt_cmps$ips, ips_others)), 
                    max(c(dt_cmps$ips, ips_others)), length.out = 11))
  hist(dt_cmps$ips, add = TRUE, col = "#E41A1C60", 
       breaks = seq(min(c(dt_cmps$ips, ips_others)), 
                    max(c(dt_cmps$ips, ips_others)), length.out = 11))
  write.csv(dt_cmps, paste0("STD_ip_", pol, ".csv"))
  #print(DT::datatable(dt_cmps, rownames = FALSE, options = list(
  #  pageLength = nrow(dt_cmps), dom = "t")))
  dt_cmps$ips <- round(dt_cmps$ips)
  dt_cmps$i1d <- round(dt_cmps$i1*100, 3)
  dt_cmps$i2d <- round(dt_cmps$i2*100, 3)
  dt_cmps$i3d <- round(dt_cmps$i3*100, 3)
  ...
  #                (dt_cmps$i2[seq(i)] > dt_cmps$not_i2[seq(i)]) |
  #                (dt_cmps$i3[seq(i)] > dt_cmps$not_i3[seq(i)]),])
}
```
