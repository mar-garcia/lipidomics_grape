...
load(paste0(study, "/data/RData/data_XCMS_POS.RData"))
...
load(paste0(study, "/data/RData/data_XCMS_NEG.RData"))
...
ions <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(ions) <- c("class", "compound", "RT", "mz", "adduct")
for(i in seq(nrow(ids))){
  ...
  for(j in seq(length(i.ions))){
    ...
    ions[nrow(ions)+1, ] <- c(ids$class[i], ids$compound[i], ids$RT[i], j.mz, i.ions[j])
  }
}
...
for(i in seq(length(cmp.class))){
  ...
  idx <- which(i.mydb$compound %in% ids$compound[which(ids$comments == "RT not sure")])
  if(length(idx) == 0){
    idx <- length(rt)+1
  }
  md <- lm(rt[-idx]~C[-idx]+db[-idx])
  plot(rt, C, col = "white", 
       main = paste0(cmp.class[i], "\n", "RT=", 
