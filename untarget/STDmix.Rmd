...
cmpsx <- cmpsx[which(cmpsx$ID_cmp == "C0612"):which(cmpsx$ID_cmp == "C0710"),]
...
for(i in 55#seq(nrow(cmps)) until 55
){
  print(cmps$ID[i])
  ...
  for(p in c("POS", "NEG")){
    ...
    plot(sps$mz, sps$irel, type = "h", xlab = "mz", ylab = "relative intensity",
         main = paste0(
           "FS at ", 
           sprintf("%.2f", rtime(xdata2[[closest(i.rt*60, rtime(xdata2))]])/60), "min"))
    tmp <- c()
    for(k in seq(0, 3)){
    ...
    if(length(ms2sub) > 0){
      ...
      for(j in seq(length(ms2sub))){
        idx <- c(which(unlist(mz(ms2sub[j])) > 283.2 & unlist(mz(ms2sub[j])) < 283.9),
                 which(unlist(mz(ms2sub[j])) > 340.7 & unlist(mz(ms2sub[j])) < 341.8))
        if(length(idx) == 0){
          int.noise <- 1
