  ...
  for(j in seq(nrow(cmps_ions))){ # ion "j"
    if((cmps_ions[j, i] == 2) | (cmps_ions[j, i] == pol)){
      ...
      if(grepl("13C", cmps_ions$ID[j])){
        if(grepl("\\(13C)", cmps_ions$ID[j])){
          ions_loop$mz <- unlist(CompoundDb::mass2mz(Rdisop::getMolecule(
        ...
        ions_loop$mz <- Rdisop::getMolecule(cmps$formula[idx])$exactmass
      }else {
        ions_loop$mz <- unlist(CompoundDb::mass2mz(Rdisop::getMolecule(
...
for(i in 1:nrow(features)){
  ...
  features$ppm[i] <- paste(sprintf("%.1f", round(abs((
