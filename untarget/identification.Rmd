...
if(polarity == "POS"){
  ...
  for(i in 1:nrow(cmps)){
    if(cmps$formula[i] != ""){
      cmps$mz[i] <- unlist(MetaboCoreUtils::mass2mz(getMolecule(
        cmps$formula[i])$exactmass, "[M+H]+"))
      cmps$NH4[i] <- unlist(MetaboCoreUtils::mass2mz(getMolecule(
        cmps$formula[i])$exactmass, "[M+NH4]+"))
      cmps$Na[i] <- unlist(MetaboCoreUtils::mass2mz(getMolecule(
    ...
    for(i in 1:nrow(cmps)){
      if(cmps$formula[i] != ""){
        cmps$mz[i] <- unlist(MetaboCoreUtils::mass2mz(getMolecule(
          cmps$formula[i])$exactmass, "[M-H]-"))
        cmps$Cl[i] <- unlist(MetaboCoreUtils::mass2mz(getMolecule(
          cmps$formula[i])$exactmass, "[M+Cl]-"))
        cmps$FA[i] <- unlist(CompoundDb::mass2mz(getMolecule(
  ...
  for(j in seq(nrow(cmps_ions))){ # ion "j"
    if((cmps_ions[j, i] == 2) | (cmps_ions[j, i] == pol)){
      ...
      if(grepl("13C", cmps_ions$ID[j])){
        if(grepl("\\(13C)", cmps_ions$ID[j])){
          ions_loop$mz <- unlist(CompoundDb::mass2mz(getMolecule(
...
if(polarity == "POS"){
  cmps_ft <- read.csv("data/compounds_ft.csv")
...
for(i in 1:nrow(features)){
  ...
  cmp.i <- ions_long[unlist(
    CompoundDb::matchWithPpm(features$mzmed[i], ions_long$mz, ppm = 15)), ]
  if(nrow(cmp.i) > 0){
    if(cmp.i$C %in% "C0015"){
      rtr[1] <- rtr[1] - 30
    }}
...
for(i in seq(nrow(features))){
  if(length(cmps$class[cmps$name == features$compound[i]]) > 0){
    features$class[i] <- cmps$class[cmps$name == features$compound[i]]
  }
}
