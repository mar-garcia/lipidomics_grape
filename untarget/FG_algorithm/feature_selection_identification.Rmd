...
for(z in seq(length(z.polarities))){
  ...
  features$solv_prop[features$solv_prop == "Inf"] <- features$mean_solv[
    features$solv_prop == "Inf"]
  features$blank_prop <- features$mean_blank / features$mean_blank_NO
  features$blank_prop[features$blank_prop == "Inf"] <- features$mean_blank[
    features$blank_prop == "Inf"]
...
for(x in seq(length(x.polarity))){
  ...
  load(paste0("data/RData/IS_data_XCMS_", x.polarity[x], ".RData"))
  load(paste0("../tissues/data/RData/MS2_library_", x.polarity[x], ".RData"))

  
  datax_tmp <- dt_preparation(xobj = xdata, class = "tissue", 
