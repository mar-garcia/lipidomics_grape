...
cmps.q <- read.csv("maturation/data/data_quanti.csv", header = F)[1,-1]
...
data <- read.csv("maturation/data/data_quanti.csv", row.names = 1)
...
class <- gsub("QC0.*", "QC", class)
class <- gsub("QC1.*", "QC", class)
```
...
text(tmp$PC1, tmp$PC2, gsub("_.*", "", gsub("Pt", "", rownames(data))), 
     pos = 1, cex = 0.6)
abline(v = 0, h = 0, lty = 2, col = "grey")
tmp <- data.frame(pca$rotation)
plot(tmp$PC1, tmp$PC2, xlab = "PC1", ylab = "PC2", bty = "l")
...
tmp <- data.frame(pca$rotation)
text(tmp$PC1*38, tmp$PC2*29, cmps.q[1,], cex = 0.5)

tmp <- data.frame(pca$x)
plot(tmp$PC1, tmp$PC2, 
     xlab = paste0("PC1 (", sprintf("%.1f", summary(pca)$importance[2,1]*100), "%)"), 
     ylab = paste0("PC2 (", sprintf("%.1f", summary(pca)$importance[2,2]*100), "%)"), 
     col = class_col[class],  bty = "l")
grid()
abline(v = 0, h = 0, lty = 2, col = "grey", lwd = 2)
tmp <- data.frame(pca$rotation)
points(tmp$PC1*38, tmp$PC2*29, xlab = "PC1", ylab = "PC2", bty = "l", pch = 8,
...
tmp$class <- droplevels(tmp$class)
