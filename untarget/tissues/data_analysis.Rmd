...
if(bygrgrape){
  ...
  class_prop <- c(1, 1, 1, 0.02974026, 0.821298701, 0.148961039)
  names(class_prop) <- c("solv", "QC", "entire", "seeds", "pulp", "skin")
  data <- t(t(data) * class_prop[class])
}
...
if(bygrgrape){
  write.csv(feat[,c(1:7, (which(colnames(feat) == "ms_level") + 1):ncol(feat))], 
          "data/features_grgrape.csv")
} else {
  write.csv(feat[,c(1:7, (which(colnames(feat) == "ms_level") + 1):ncol(feat))], 
          "data/features_grtissue.csv")
}
