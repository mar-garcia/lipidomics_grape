...
i <- 1
print(cmps$name[i])
cmps.mx.i <- matrix(cmps.mx[i, ], nrow = 1)
colnames(cmps.mx.i) <- c("mzmin", "mzmax", "rtmin", "rtmax")
peaks <- manualChromPeaks(data_raw,
                          chromPeaks = cmps.mx.i,
                          samples = seq_along(fileNames(data_raw)),
                          BPPARAM = bpparam(),
                          msLevel = 1L)
data <- data.frame(chromPeaks(peaks))
data$ppm <- ((data$mz - cmps$mz[i])/cmps$mz[i])*1e6

plot_ly(y = data$rt/60, x = data$sample, 
        color = factor(data_raw$color[data$sample]), 
        text = substr(data_raw$filename[data$sample], 17, 28)) %>% 
