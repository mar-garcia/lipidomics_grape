...
For that I´ll use the following types of samples: `solvent` (we shouldn't see "anything"), `blank` (we should see "only" the IS) and `STDmix` (we should see the ions related with the 77 compounds included in this mix). There are 2 injections of each type of sample. Therefore, we will use 6 samples in total.  
...
#  Get filenames
if(study == "maturation"){
  injections <- list.files(paste0(study, "/data/", polarity, "_FS_round1_fixed/"))
  injections <- data.frame(
    filename = injections[grepl("blank|STDmix|slv_01|slv_02", injections)])
} else if(study == "tissues"){
...
if(study == "maturation"){
  colnames(tmp) <- c("order", "project", "pt", "time", "type", "rep", 
                     "polarity", "mode")
  tmp$time[tmp$time == "00"] <- tmp$type[tmp$time == "00"]
} else if(study == "tissues"){
...
if(study == "maturation"){
  data_raw <- readMSData(
    paste0(study, "/data/", polarity, "_FS_round1_fixed/" , injections$filename),
...
pgp <- PeakGroupsParam(span = 0.3,
                       subset = which(pData(xdata)[,v.class] == "STDmix"),
...
datax <- dt_preparation(xobj = xdata, class = v.class, 
                        mznoise = c(158.975, 184.0731, 219.174, 221.154, 
                                    251.164, 267.196, 269.2115, 283.699, 
                                    415.284))

#features <- datax[["features"]]
#features <- features[features$npeaks == 2, ]
#features <- features[
#  apply(features[,grepl("prop_", colnames(features))], 1, max) > thr_int, ]
...
datax <- ft_grouping(
  datax = datax, 
  MS2x = ms2list,
  ft_idx = which(features$npeaks == 2 & 
                   apply(features[,grepl("prop_", colnames(features))], 1, 
                         max) > thr_int))
...
} else if(nrow(cmp_id) == 3){
  rownames(cmp_id) <- c("unknown", "identified", "duplicated")
  cmp_id$Freq[2] <- cmp_id$Freq[2] + (cmp_id$Freq[3]/2)
  cmp_id$Freq[3] <- cmp_id$Freq[3] - (cmp_id$Freq[3]/2)
}
...
save.image(paste0("RT_ppm_dev_", study, "_", polarity, ".RData"))
```
