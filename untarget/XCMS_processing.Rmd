...
injections <- injections[grepl("entire", injections$tissue),]
...
pgp <- PeakGroupsParam(span = 0.3,
                       #subset = which(pData(xdata)[,v.class] == "QC" | 
                       #                pData(xdata)[,v.class] == "xQC"),
...
plot(chr_raw, col = paste0(col_type[pData(xdata)[,v.class]], 40),
     peakType = "none", main = "BPC, raw")
plot(chr_adj, col = paste0(col_type[pData(xdata)[,v.class]], 40),
     peakType = "none", main = "BPC, adjusted")
plotAdjustedRtime(xdata, col = paste0(col_type[pData(xdata)[,v.class]], 80))
```
...
if(study == "maturation"){
  dt <- data[pData(xdata)[,v.class] == "study",]
} else if(study == "tissues"){
  dt <- data[pData(xdata)[,v.class] %in% c("entire", "skin", "pulp", "seeds"),]
}
dt <- log10(dt)
dt <- data.frame(scaling.pareto(dt))
pca <- prcomp(dt, center = FALSE, scale. = FALSE)
...
```{r save}
save(xdata, file = paste0(study, "/data/RData/data_XCMS_entire_", polarity, ".RData"))
```
