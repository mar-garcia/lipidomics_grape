...
res <- data.frame(matrix(nrow = length(cmp), ncol = 12))
colnames(res) <- c("compound", "RT", "t_ions", "m_ions", "cor_i", "cor_ps",
                   "ip_NEG", "ip_POS", 
                   "sim_MS2", "sim_NEG_std", "sim_POS_std", "class")
res$compound <- cmp
for(i in seq(length(cmp))){
  ...
  for(p in c("POS", "NEG")){
    if(sum(dbi$polarity == p) > 1){
      if(min(round(dbi$sim[dbi$polarity == p], 3)) ==
         max(round(dbi$sim[dbi$polarity == p], 3))){
        res[i, paste0("sim_", p, "_std")] <- sprintf(
          "%.3f", mean(dbi$sim[dbi$polarity == p]))
      } else {
           res[i, paste0("sim_", p, "_std")] <- paste(
        sprintf("%.3f", range(dbi$sim[dbi$polarity == p])), collapse  = "-")
         }
    }else if(sum(dbi$polarity == p) == 1){
      res[i, paste0("sim_", p, "_std")] <- sprintf(
        "%.3f", as.numeric(dbi$sim[dbi$polarity == p]))
    } else {
